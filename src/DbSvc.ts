...
const INSERT_PLAYER_SQL = "INSERT INTO player (name) VALUES($1)"
...
const UPDATE_MINIGAME_SQL = "UPDATE mini_game SET mini_game.name = $1, maxscore = $2 WHERE id=$3"
const GET_MINIGAMES_SQL = "SELECT * FROM mini_game"
...
export class DbSvc {
    ...
    async insertPlayer(player: Player) : Promise<Player> {
        await this.pool.query(INSERT_PLAYER_SQL, [player.name])
        return player
    }
    ...
    async insertMiniGame(game: MiniGame) : Promise<MiniGame> {
        await this.pool.query(INSERT_MINIGAME_SQL, [game.name, game.maxscore])
        return game
    }
